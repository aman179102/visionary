...
type Prediction = blazeface.NormalizedFace;
...
export function FaceDetector() {
  ...
  const drawBoundingBoxes = useCallback((detectedFaces: Prediction[]) => {
    ...
    detectedFaces.forEach((face) => {
      ...
      const text = `Face (${(face.probability[0] as number * 100).toFixed(1)}%)`;
      const textWidth = ctx.measureText(text).width;
      ctx.fillRect(start[0], start[1] > 20 ? start[1] - 20 : start[1], textWidth + 8, 20);

      // Label text
      ctx.fillStyle = "hsl(var(--primary-foreground))";
      ctx.font = "12px 'Inter', sans-serif";
      ctx.fillText(text, start[0] + 4, start[1] > 20 ? start[1] - 6 : start[1] + 14);
    });
  }, [activeTab]);
    ...
    const loadModel = async () => {
      try {
        ...
        toast({
          variant: "destructive",
          title: "Model Error",
          description: "Failed to load the BlazeFace model. Please refresh the page.",
        });
      } finally {
  ...
  const detectLoop = useCallback(async () => {
    if (
      ...
      try {
        const detectedFaces = await model.estimateFaces(videoRef.current, false);
        setPredictions(detectedFaces);
  ...
  const detectUploadedImage = useCallback(async () => {
    if (model && imageRef.current) {
      try {
        const detectedFaces = await model.estimateFaces(imageRef.current, false);
        setPredictions(detectedFaces);
        drawBoundingBoxes(detectedFaces);
      } catch (error) {
        ...
        toast({
          ...
          description: "Could not detect faces in the uploaded image.",
        });
      }
    }
  ...
  if (isLoadingModel) {
    return (
      <div className="flex flex-col items-center justify-center h-96 gap-4 text-primary">
        ...
        <p className="text-lg font-semibold">Loading Face Detection Model...</p>
      ...
      <div className="lg:col-span-2">
        <Card>
          <CardHeader className="text-center">
            <CardTitle className="text-3xl font-bold text-primary">BlazeFace Face Detection</CardTitle>
            <CardDescription className="text-lg text-muted-foreground pt-2">
              Detects faces in images or a live webcam stream.
            </CardDescription>
          </CardHeader>
        </Card>
            ...
            {predictions.length > 0 ? (
              <div className="space-y-2">
                <div className="flex items-center justify-between p-2 rounded-md bg-secondary">
                    <span className="font-semibold text-secondary-foreground">{predictions.length} {predictions.length > 1 ? 'Faces' : 'Face'} Detected</span>
                </div>
                {predictions.map((prediction, index) => (
                  <div key={index} className="flex items-center justify-between p-2 rounded-md bg-secondary/50">
                    <span className="font-semibold capitalize text-secondary-foreground">Face #{index + 1}</span>
                    <Badge variant="outline">
                      {`${((prediction.probability as number)[0] * 100).toFixed(1)}%`}
                    </Badge>
                  </div>
                ))}
              </div>
                ...
                <Alert>
                  ...
                  <AlertDescription>
                    {activeTab === 'webcam' ? 'Start the webcam to detect faces in real-time.' : 'Upload an image to see the detected faces.'}
                  </AlertDescription>
                </Alert>
            )}
          </CardContent>
